...
import static zzzqi.keccak.HexUtils.leftRotate64;
import static zzzqi.keccak.HexUtils.convertToUint;
import static zzzqi.keccak.HexUtils.convertFromLittleEndianTo64;
import static zzzqi.keccak.HexUtils.convertFrom64ToLittleEndian;
...
public class Keccak extends Activity {
    ...
    public byte[] getHash(final byte[] message, final Parameters parameter) {
        ...
        int[] uMessage = convertToUint(message);//°Ñbyte×ª±äÎªunsignedÎÞ·ûºÅ


        int rateInBytes = parameter.getRate() / 8;//±ÈÌØÂÊ³¤¶È±äÎª×Ö½Ú³¤¶ÈÐÎÊ½
        int blockSize = 0;	//¿é³¤¶È
        int inputOffset = 0;//Ã÷ÎÄ³¤¶ÈµÄÆ«ÒÆ
